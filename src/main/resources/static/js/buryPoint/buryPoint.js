					...
					$.ajax({
				        ...
				        success : function(rs) {
				        	...
				        	if(code == '200'){
				        		...
				        		layer.close(index);
				        		queryBuryRcd(dfType,dfBuryType,$("#stageSelect").text());
				        	}else{
 ...
 * @returns
 */
function alterBuryPointClick(){
	$("#alterBuryPoint").click(function(){
		var checks = $("#buryTable").find("tbody input[type=checkbox]:checked");
		if(checks.length == 0){
			layer.msg("请选择要修改的埋点！",{icon:2});
			return;
		}else{
			var ids = "";
			checks.each(function(){
			...
			$.ajax({
		        ...
		        success : function(rs) {
		        	...
		        	if(dfType == '1'){//通用/主题相册埋点
		        		var area = ['100%', '100%'];
		    			showBuryAlterWin("#pointDivAlter",data,area);
		    		}else if(dfType == '2'){//促活
		    			showBuryAlterWin("#pointPushDivAlter",data,area);
		    		}else if(dfType == '3'){//底图调取
		    			var area = ['70%', '85%'];
		    			showBuryAlterWin("#pointDrawDivAlter",data,area);
		    		}else if(dfType == '4'){//登录信息统计
		    			var area = ['62%', '85%'];
		    			showBuryAlterWin("#pointLoginDivAlter",data,area);
		    		}
 ...
 * @returns
 */
function showBuryAlterWin(dom,data,area){
	$(dom).find(".addul").empty();
	var html = $(dom).find(".example").html();
	for(var i in data){
		var item  = data[i];
		var liHtml = html;
		for(var x in item){
			liHtml = liHtml.replace("{"+x+"}",item[x]);
		}
		$(dom).find(".addul").append(liHtml);
	}
	index = layer.open({
        type: 1,
        title:"修改埋点-"+dfBuryType,
        area: area,
...
function addOrDeletePort(){
	...
	//修改
	$("#alterPort").click(function(){
		var checks = $("#portTable").find("tbody input[type=checkbox]:checked");
		if(checks.length == 0){
			layer.msg("请选择要删除的渠道！",{icon:2});
			return;
		}else if(checks.length > 1){
			layer.msg("请选择一条修改的记录！",{icon:2});
			return;
		}else{
			var id = checks.eq(0).val();
			$.ajax({
		        type : "POST",  //请求方式
		        url : "/alterPortData",  //请求路径
		        data:{
		        	id:id
		        },
		        async:true,
		        success : function(rs) {
		        	$("#alterPortDiv").find("input[type=text]").val("");
		        	$("#alterPortDiv").find("textarea").val("");
		        	var data = rs.data;
		        	//打开修改窗
		        	if(!data){
		        		return;
		        	...
		        	for(var x in data){
		        		$("#alterPortDiv").find("."+x).val(data[x]);
		        	}
		        	index = layer.open({
			            type: 1,
	...
	$(".portAlterSave").click(function(){
		var postData = {};
		$("#alterPortDiv").find("input,textarea").each(function(){
			postData[$(this).attr("name")] = $(this).val();
		});
		$.ajax({
	        type : "POST",  //请求方式
	        url : "/saveAlterPortData",  //请求路径
	        data:postData,
	        async:true,
	        success : function(rs) {
	        	var code = rs.code;
...
function queryBuryPoint(type,buryType){
	...
	$.ajax({
        ...
        success : function(rs) {
        	...
        	$("#stageSelect").text("--全部--");
        	$("#stageSelect").parent().find("ul").append("<li onclick='setVal(this,\"--全部--\")'><a href='javascript:void(0);'>--全部--</a></li>");
        	for(var x in data){
